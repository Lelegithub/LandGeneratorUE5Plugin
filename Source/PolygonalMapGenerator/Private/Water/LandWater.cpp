...
ULandWater::ULandWater()
{
	WaterCutoff = 0.0f;
	bInvertLandAndWater = false;
}

void ULandWater::AssignOcean_Implementation(TArray<bool>& r_ocean, UTriangleDualMesh* Mesh, const TArray<bool>& r_water) const
{
...
void ULandWater::AssignWater_Implementation(TArray<bool>& r_water, FRandomStream& Rng, UTriangleDualMesh* Mesh, const FIslandShape& Shape) const
{
	if (Mesh)
	{
#if !UE_BUILD_SHIPPING
...
void ULandWater::InitializeWater_Implementation(TArray<bool>& r_water, UTriangleDualMesh* Mesh, FRandomStream& Rng) const
{
	// Empty
}

bool ULandWater::IsPointLand_Implementation(FPointIndex Point, UTriangleDualMesh* Mesh, const FVector2D& HalfMeshSize, const FVector2D& Offset, const FIslandShape& Shape) const
{
	return false;
}

void ULandWater::assign_r_water(TArray<bool>& r_water, FRandomStream& Rng, UTriangleDualMesh* Mesh, const FIslandShape& Shape) const
{
	AssignWater(r_water, Rng, Mesh, Shape);
}

void ULandWater::assign_r_ocean(TArray<bool>& r_ocean, UTriangleDualMesh* Mesh, const TArray<bool>& r_water) const
{
